...
if (isset($_SESSION['custom_vars']) && !empty($_SESSION['custom_vars'])) {
  ...
  if (isset($cvar[0]) && !empty($cvar[0])) $_REQUEST["start_name"] = $cvar[0];
  if (isset($cvar[1]) && !empty($cvar[1])) $_REQUEST["start_email"] = $cvar[1];
  if (isset($cvar[2]) && !empty($cvar[2])) $_REQUEST["start_chat_msg"] = $cvar[2];
}

// Let's get the operators
$online_oplist = online_operator_list($opcacheid, $LC_DEPARTMENTS, $jakwidget[$widgetid]['depid'], $jakwidget[$widgetid]['opid']);
