...
$usrfolder = $usrfolder2 = $usrfolder3 = '';

// No superadmin access?
if (!JAK_SUPERADMINACCESS) {
    $usrfolder = $opcacheid.'/';
    $usrfolder2 = '/'.$opcacheid.'/';
    $usrfolder3 = '/'.$opcacheid;
}
...
switch ($page1) {
    ...
    case 'delete':
        ...
        if (!empty($page2)) {
            ...
            $filedel = APP_PATH.JAK_FILES_DIRECTORY.'/buttons/'.$usrfolder.$page2;
            if (file_exists($filedel)) {
                unlink($filedel);
            }

            $filedel1 = APP_PATH.JAK_FILES_DIRECTORY.'/buttons/'.$usrfolder.$offbtn;
            if (file_exists($filedel1)) {
                unlink($filedel1);
            }

            // Write the log file each time someone tries to login before
            JAK_base::jakWhatslog('', $opcacheid, JAK_USERID, 0, 95, 0, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
    ...
    case 'deletef':
        ...
        if (!empty($page2)) {
            ...
            // Now let us delete the file
            $filedel = APP_PATH.JAK_FILES_DIRECTORY.'/slideimg/'.$usrfolder.$page2;
            if (file_exists($filedel)) {
                unlink($filedel);
            }

            $filedel1 = APP_PATH.JAK_FILES_DIRECTORY.'/slideimg/'.$usrfolder.$offbtn;
            if (file_exists($filedel1)) {
                unlink($filedel1);
            }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    	...
    	if (($_FILES['uploadpp']['name'] != '' && $_FILES['uploadpp1']['name'] != '') || ($_FILES['uploadppsi']['name'] != '' && $_FILES['uploadppsi1']['name'] != '')) {
            ...
            if ($_FILES['uploadpp']['name'] != '' && $_FILES['uploadpp1']['name'] != '') {
        	   ...
        	   $filename1 = $_FILES['uploadpp1']['name']; // original filename
                ...
                $targetPath = APP_PATH.JAK_FILES_DIRECTORY.'/buttons/'.$usrfolder;
                $abstargetPath = APP_PATH.JAK_FILES_DIRECTORY.'/buttons/'.$usrfolder3;
                ...
                // first get the target path
                $targetPath = APP_PATH.JAK_FILES_DIRECTORY.'/slideimg/'.$usrfolder;
                $abstargetPath = APP_PATH.JAK_FILES_DIRECTORY.'/slideimg/'.$usrfolder3;
            }

            // Create directory if not exist
            if (!is_dir($abstargetPath)) {
                mkdir($abstargetPath, 0755);
                copy(APP_PATH.JAK_FILES_DIRECTORY."/index.html", $abstargetPath."/index.html");
            }
        ...
        if (count($errors) == 0) {
            ...
            // Write the log file each time someone tries to login before
            JAK_base::jakWhatslog('', $opcacheid, JAK_USERID, 0, 94, 0, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
    ...
    // Get all buttons from the client
    if (!JAK_SUPERADMINACCESS) {
        if (is_dir(APP_PATH.JAK_FILES_DIRECTORY.'/buttons'.$usrfolder3)) $BUTTONS_ClIENT = jak_get_files(APP_PATH.JAK_FILES_DIRECTORY.'/buttons'.$usrfolder3);

        // Get all slideup images from the client
        if (is_dir(APP_PATH.JAK_FILES_DIRECTORY.'/slideimg'.$usrfolder3)) $SLIDEIMG_CLIENT = jak_get_files(APP_PATH.JAK_FILES_DIRECTORY.'/slideimg'.$usrfolder3);
    }
