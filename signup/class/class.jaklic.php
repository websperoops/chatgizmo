...
define("LB_TEXT_INVALID_RESPONSE", 'Invalid license, please register bellow or contact <a class="alert-link" href="https://jakweb.ch">support</a>.');
define("LB_TEXT_VERIFIED_RESPONSE", 'Verified! Thanks for purchasing Cloud Chat 3.');
...
class JAKLicenseAPI{
	...
	public function __construct(){ 
		...
		$this->root_path = str_replace("admin".DIRECTORY_SEPARATOR, "", APP_PATH).'process/updates/';
		$this->license_file = str_replace("admin".DIRECTORY_SEPARATOR, "", APP_PATH).'process/updates/.lic';
	}
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["10b869aeb50f55f"])){
				$_SESSION["10b869aeb50f55f"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["10b869aeb50f55f"])){
				$get_data = $this->call_api(
					'POST',
	...
	public function download_update($update_id, $type, $version, $license = false, $client = false){ 
		...
		if($res === TRUE){
			$zip->extractTo(str_replace("admin/", "", APP_PATH)); 
			$zip->close();
			unlink($destination);
		...
		if($type == true){
			...
			$dbupdatefile = str_replace("admin/", "", APP_PATH).'update.php';
