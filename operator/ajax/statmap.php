...
if (isset($_POST["depid"]) && is_numeric($_POST["depid"]) && $_POST["depid"] != 0) {
	...
	$maparray = $jakdb->select("sessions", ["id", "countrycode", "country", "city", "name", "latitude", "longitude"], ["AND" => ["opid" => $opcacheid, "department" => $_POST["depid"], "longitude[!]" => ''], "ORDER" => ["id" => "DESC"], "LIMIT" => 2000]);

} else {

	$maparray = $jakdb->select("sessions", ["id", "countrycode", "country", "city", "name", "latitude", "longitude"], ["AND" => ["opid" => $opcacheid, "longitude[!]" => ''], "ORDER" => ["id" => "DESC"], "LIMIT" => 2000]);
}
