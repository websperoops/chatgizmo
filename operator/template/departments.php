		...
		<div class="card-header">
	  	...
	  	<div class="card-body">
			...
			<table class="table table-striped">
				...
				<tbody>
					<?php foreach($DEPARTMENTS_ALL as $i => $v) { ?>
					<tr>
						<td><input type="hidden" name="real_dep_id[]" value="<?php echo $v["id"];?>"></td>
						...
						<td><?php if ($totalAll > 1) { ?><a class="btn btn-default btn-sm" href="<?php echo JAK_rewrite::jakParseurl('departments', 'lock', $v["id"]);?>"><i class="fa fa-<?php if ($v["active"] == '1') { ?>check<?php } else { ?>lock<?php } ?>"></i></a><?php } ?></td>
...
<div class="col-md-3">
	...
	<?php } else if (!empty($jakosub) && $jakosub['trial'] == 1) { ?>
  <div class="alert alert-info"><?php echo $jkl['i11'];?></div>
  <?php } else { ?>
  <div class="alert alert-info"><?php echo sprintf($jkl['i14'], $totalAll, $jakosub['departments'], $jkl['m9']);?></div>
  <?php if ($totalAll > 1 && $totalAll > $jakosub['departments']) { ?>
  <div class="alert alert-danger"><?php echo sprintf($jkl['i64'], $delW);?></div>
  <?php } } ?>
</div>	
</div>
