...
if (!empty($userid) && !empty($loginhash) && !empty($chatid) && is_numeric($chatid)) {
	...
	if ($usr) {
    	...
    	include_once APP_PATH.JAK_CACHE_DIRECTORY.'/opcache'.$opcacheid.'.php';
		...
		if ($task == "respond") {
			...
			if (count($errors) == 0) {
				...
				$row = $jakdb->get("sessions", ["name", "operatorname", "department"], ["AND" => ["id" => $chatid, "opid" => $opcacheid]]);
		...
		if ($task == "file") {
			...
			$sendfile = $jakdb->get("checkstatus", "files", ["AND" => ["convid" => $chatid, "opid" => $opcacheid]]);

			if ($sendfile == 1) {
				$jakdb->update("checkstatus", ["files" => 0], ["AND" => ["convid" => $chatid, "opid" => $opcacheid]]);
				// File upload off 
				die(json_encode(array('status' => true, 'task' => "files", 'upload' => false)));
					
			} else {
				$jakdb->update("checkstatus", ["files" => 1], ["AND" => ["convid" => $chatid, "opid" => $opcacheid]]);
				// File upload on 
		...
		if ($task == "end") {
			...
			$row = $jakdb->get("sessions", ["id", "name", "email"], ["AND" => ["id" => $chatid, "opid" => $opcacheid]]);

			$jakdb->update("sessions", ["status" => 0, "ended" => time()], ["AND" => ["id" => $chatid, "opid" => $opcacheid]]);
		    $jakdb->update("checkstatus", ["hide" => 1], ["AND" => ["convid" => $row['id'], "opid" => $opcacheid]]);
		...
		if ($task == "knock") {
			...
			$result = $jakdb->update("checkstatus", ["knockknock" => 1], ["AND" => ["convid" => $chatid, "opid" => $opcacheid]]);
		...
		if ($jakuser->getVar("transferc") == 1) {
			...
			$result = $jakdb->select("user", ["id", "departments", "username", "name", "available", "lastactivity"], ["AND" => ["OR" => ["id" => $opcacheid, "opid" => $opcacheid], "id[!]" => $userid, "access" => 1, "available" => 1]]);

			// Get departments
			$lsdata = $jakdb->select("departments", ["id", "title"], ["opid" => $opcacheid, "ORDER" => ["dorder" => "ASC"]]);
