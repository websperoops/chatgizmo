...
switch ($_POST['oprq']) {
	case 'status':
		...
		if ($_POST['available'] == 1) {
			...
			$jakdb->update("user", ["available" => 1], ["OR" => ["id" => $_POST['uid'], "opid" => $opcacheid]]);
			$row = $jakdb->get("user",["operatorlist", "operatorchat"], ["AND" => ["OR" => ["id" => $_POST['uid'], "opid" => $opcacheid], "access" => 1]]);
			...
			$row = $jakdb->get("user",["operatorlist", "operatorchat"], ["AND" => ["id" => $_POST['uid'], "opid" => $opcacheid, "access" => 1]]);
