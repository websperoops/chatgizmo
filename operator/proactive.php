...
if (!jak_get_access("proactive", $jakuser->getVar("permissions"), JAK_MAIN_OP)) jak_redirect(BASE_URL);
...
switch ($page1) {
	...
	case 'delete':
		...
		if (!$result) {
			...
			// Write the log file each time someone tries to login before
          	JAK_base::jakWhatslog('', $opcacheid, JAK_USERID, 0, 81, $page2, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
		...
		if (is_numeric($page2) && jak_row_exist($page2, $opcacheid, $jaktable, JAK_MAIN_OP)) {
		    ...
		    if (count($errors) == 0) {
		    	...
		    	$result = $jakdb->update($jaktable, ["path" => $jkp['path'],
					...
					"btn_cancel" => $jkp['btn-c']], ["AND" => ["id" => $page2, "opid" => $opcacheid]]);
				...
				if (!$result) {
					...
					// Write the log file each time someone tries to login before
          			JAK_base::jakWhatslog('', $opcacheid, JAK_USERID, 0, 79, $page2, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
	...
	default:
		    ...
		    if (!filter_var($jkp['path'], FILTER_VALIDATE_URL)) {
		        ...
		        if (count($errors) == 0) {
		    		...
		    		if (!$lastid) {
		    			...
		    			// Write the log file each time someone tries to login before
          				JAK_base::jakWhatslog('', $opcacheid, JAK_USERID, 0, 80, $lastid, (isset($_COOKIE['WIOgeoData']) ? $_COOKIE['WIOgeoData'] : ''), $jakuser->getVar("username"), $_SERVER['REQUEST_URI'], $ipa, $valid_agent);
		...
		// Get all proactive
		$PROACTIVE_ALL = jak_get_page_info($jaktable, $opcacheid);

		// How often we had changes
	    $totalChange = $jakdb->count("whatslog", ["AND" => ["opid" => $opcacheid, "whatsid" => [79,80,81]]]);

	    // Last Edit
	    if ($totalChange != 0) {
	      $lastChange = $jakdb->get("whatslog", "time", ["AND" => ["opid" => $opcacheid, "whatsid" => [79,80,81]], "ORDER" => ["time" => "DESC"], "LIMIT" => 1]);
	    }
