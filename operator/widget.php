...
switch ($page1) {
	...
	case 'delete':
		...
		if (!$result) {
		    ...
		    jak_redirect(JAK_rewrite::jakParseurl('widget'));
		} else {
	...
	case 'edit':
		...
		if (is_numeric($page2) && jak_row_exist($page2, $opcacheid, $jaktable, JAK_MAIN_OP)) {
			...
			if (isset($JAK_FORM_DATA["btn_tpl"]) && !empty($JAK_FORM_DATA['btn_tpl'])) {
				...
				$btn_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "btn_form", $btnsett["formoptions"], $BUTTONS_ALL, $SLIDEIMG_ALL);
			}
			...
			if (isset($JAK_FORM_DATA["start_tpl"]) && !empty($JAK_FORM_DATA['start_tpl'])) {
				...
				// Get the options for the selected button
				$start_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "start_form", $startsett["formoptions"], "", "");
			}
			...
			if (isset($JAK_FORM_DATA["chat_tpl"]) && !empty($JAK_FORM_DATA['chat_tpl'])) {
				...
				// Get the options for the selected button
				$chat_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "chat_form", $chatsett["formoptions"], "", "");
			}
			...
			if (isset($JAK_FORM_DATA["contact_tpl"]) && !empty($JAK_FORM_DATA['contact_tpl'])) {
				...
				// Get the options for the selected button
				$contact_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "contact_form", $contactsett["formoptions"], "", "");
			}
			...
			if (isset($JAK_FORM_DATA["profile_tpl"]) && !empty($JAK_FORM_DATA['profile_tpl'])) {
				...
				// Get the options for the selected button
				$profile_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "profile_form", $profilesett["formoptions"], "", "");
			}
			...
			if (isset($JAK_FORM_DATA["feedback_tpl"]) && !empty($JAK_FORM_DATA['feedback_tpl'])) {
				...
				// Get the options for the selected button
				$feedback_form = jak_get_custom_fields($opcacheid, $page2, $JAK_FORM_DATA['template'], "feedback_form", $feedbacksett["formoptions"], "", "");
			}
	...
	default:
		...
		if ($jakosub['chatwidgets'] <= $totalAll) $newwidg = false;
		...
		// Get all operators
		$JAK_OPERATORS = $jakdb->select($jaktable2, ["id", "username"], ["OR" => ["id" => JAK_USERID, "opid" => $opcacheid], "ORDER" => ["username" => "ASC"]]);

		// Call the settings function
		$lang_files = jak_get_lang_files();

		// How often we had changes
		$totalChange = $jakdb->count("whatslog", ["AND" => ["opid" => $opcacheid, "whatsid" => [17,18,33]]]);

		// Last Edit
		if ($totalChange != 0) {
			$lastChange = $jakdb->get("whatslog", "time", ["AND" => ["opid" => $opcacheid, "whatsid" => [17,18,33]], "ORDER" => ["time" => "DESC"], "LIMIT" => 1]);
		}

		// Now we have a downgrade
		if (!$newwidg && $totalAll > 1 && $totalAll > $jakosub['chatwidgets']) {

			if ($jakosub['chatwidgets'] == 0) {
				$delW = $totalAll - 1;
			} else {
				$delW = $totalAll - $jakosub['chatwidgets'];
			}
		}
