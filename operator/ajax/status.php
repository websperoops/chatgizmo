	...
	if (isset($_GET['olist']) && $_GET['olist']) {
		...
		if (isset($resop) && !empty($resop)) {
			...
			foreach ($resop as $row) {
				...
				$oponline .= '<li class="list-group-item">'.$row['name'].' <div class="btn-group btn-group-sm"><a href="'.str_replace('ajax/', '', JAK_rewrite::jakParseurl('uonline', 'opstat', $row['id'])).'" data-toggle="modal" data-target="#jakModal" class="btn btn-secondary"><i class="fa fa-chart-bar"></i></a>'.$openop.'</div></li>';
